...
`experiments/zero_param_origin.py` (that model is the D=1, single-modulus special
case). The pairwise CRT below is verified by the test suite (`tests/test_phase_native.py`).
"""
...
def crt_pair(a1: int, m1: int, a2: int, m2: int) -> int:
    ...
    Standard CRT construction; the package is standalone (no external CRT dependency).
    """
    inv_m1 = pow(m1, -1, m2)
    inv_m2 = pow(m2, -1, m1)
    M = m1 * m2
        ...
        for j in range(i + 1, len(moduli)):
            if gcd(moduli[i], moduli[j]) != 1:
                raise ValueError(
                    ...
                    f"share a factor. (CRT requires pairwise-coprime moduli.)"
                )
