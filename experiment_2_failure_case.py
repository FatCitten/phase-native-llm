"""
Experiment 2: Failure Case (CRT Composition)
============================================
Test if network implements NUMBER THEORY, not statistics.

CRT requires gcd(a,b) = 1 for composition to work.
Z_2 x Z_2 does NOT compose to Z_4 (gcd=2)
Z_4 x Z_9 DOES compose to Z_36 (gcd=1)

This is the GATE experiment - if Z_2 x Z_2 succeeds,
...
def main():
    print("="*60)
    print("EXPERIMENT 2: FAILURE CASE (CRT COMPOSITION)")
    print("="*60)
    
    compositions = [
        {'name': 'Z_2 x Z_2 -> Z_4', 'k1': 2, 'k2': 2, 'k_target': 4, 'gcd': 2, 'expected': 'FAIL'},
        {'name': 'Z_3 x Z_3 -> Z_9', 'k1': 3, 'k2': 3, 'k_target': 9, 'gcd': 3, 'expected': 'FAIL'},
        {'name': 'Z_4 x Z_9 -> Z_36', 'k1': 4, 'k2': 9, 'k_target': 36, 'gcd': 1, 'expected': 'SUCCEED'},
        {'name': 'Z_4 x Z_25 -> Z_100', 'k1': 4, 'k2': 25, 'k_target': 100, 'gcd': 1, 'expected': 'SUCCEED'},
    ]
    ...
    for comp in compositions:
        print(f"\n=== {comp['name']} (gcd={comp['gcd']}, expected={comp['expected']}) ===")
        ...
        status = "PASS" if (comp['expected'] == 'FAIL' and mean_closure < 0.2) or \
                          (comp['expected'] == 'SUCCEED' and mean_closure > 0.85) else "FAIL"
        
        print(f"  Mean closure rate: {mean_closure:.2%} [{status}]")
    
    print("\n" + "="*60)
    print("TABLE 1: CRT COMPOSITION MATRIX")
    print("="*60)
    print("\n| Composition  | gcd | Predicted | Actual | Status |")
    print("|--------------|-----|-----------|--------|--------|")
    
    all_pass = True
    for name, r in results.items():
        pred = r['expected']
        actual = "SUCCEED" if r['mean_closure'] > 0.5 else "FAIL"
        status = "OK" if (pred == actual) else "ANOMALY"
        if pred != actual:
            all_pass = False
        print(f"| {name:14s} | {r['gcd']:3d} | {pred:9s} | {actual:7s} | {status:6s} |")
    
    print("\n" + "="*60)
    
    if all_pass:
        print("*** FAILURE CASE CONFIRMED ***")
        print("Network implements NUMBER THEORY, not statistics!")
        print("Success exactly when gcd(a,b)=1, fail when gcd(a,b)!=1")
    else:
        print("*** ANOMALY DETECTED ***")
        print("Some predictions do not match. See table above.")
